...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void findProductsWithCategories(){
		List<Product> found = em.createQuery("select p from Product p INNER JOIN FETCH p.categories pc",Product.class).getResultList();
	...
	public void findProductsAddedYesterday() {
		...
		List<Product> products= em.createQuery("Select p from Product p where addedDate = :date ",Product.class).setParameter("date", cal.getTime().toInstant().atZone(ZoneId.systemDefault()).toLocalDate()).getResultList();
	...
	public void leftJoinCagesWithPets() {
		...
		// TODO I think there is some problem with this exercise, or I am just not able to figure out what did I wrong here...
		List<ProductAndCategory> productAndCategory = em.createQuery("Select new cz.fi.muni.pa165.tasks.ProductAndCategory(p, c) from Product p left join p.categories c ON c.name = 'Kitchen'",ProductAndCategory.class).getResultList();
